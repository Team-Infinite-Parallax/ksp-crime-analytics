...
/**
 * Top navigation bar — responsive on all screen sizes.
 * Hamburger menu toggles the sidebar drawer.
 *
 * @param {Object} props
 * @param {Function} props.onMenuToggle - Toggle sidebar drawer
 */
export default function Navbar({ onMenuToggle }) {
  ...
  return (
    <header className="h-14 sm:h-16 border-b border-[var(--color-hairline-dark)] bg-[var(--color-surface-navbar)] flex items-center justify-between px-2 sm:px-4 md:px-6 lg:px-8 z-40 relative shrink-0">
        ...
        <div className="flex items-center flex-1 min-w-0">
          <div className="relative w-full group">
            ...
            <input
              ...
              className="w-full bg-[var(--color-surface-elevated-dark)] border border-[var(--color-hairline-dark)] text-[var(--color-on-dark)] rounded-lg py-2.5 pl-10 pr-3 text-[14px] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] placeholder-[var(--color-muted)] transition-all duration-200 h-[40px] shadow-inner"
            />
          </div>
      ...
      {/* Right section: role, theme, alerts, profile */}
      <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-4 lg:space-x-6 shrink-0 ml-2">
        {/* Role selector: hidden on xs, visible sm+ */}
        <div className="hidden sm:flex items-center space-x-2 bg-[var(--color-surface-elevated-dark)] border border-[var(--color-hairline-dark)] rounded-lg px-2.5 py-1.5 sm:px-3">
          <label htmlFor="role-select" className="text-[12px] text-[var(--color-muted)] font-bold uppercase tracking-wider hidden md:inline">Role:</label>
          <select
            id="role-select"
            value={activeRole}
          ...
          <span className="text-[10px] text-[var(--color-primary)] font-bold uppercase tracking-wider border-l border-[var(--color-hairline-dark)] pl-2 ml-1 hidden lg:inline-block">For Demo</span>
        </div>
        ...
        <div className="flex items-center space-x-1 sm:space-x-3">
          <button
            ...
            className="p-2 sm:p-2.5 bg-transparent text-[var(--color-muted)] hover:text-[var(--color-on-dark)] hover:bg-[var(--color-surface-elevated-dark)] rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:outline-none"
            title="Toggle Theme"
          >
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center space-x-2 sm:space-x-3 hover:bg-[var(--color-surface-elevated-dark)] p-1 sm:p-1.5 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:outline-none"
          >
            <div className="h-7 w-7 sm:h-8 sm:w-8 bg-[var(--color-primary)] rounded-[50%] flex items-center justify-center text-white font-bold text-[12px] shrink-0">
              {userDetails.firstName ? userDetails.firstName.charAt(0) : 'P'}
            </div>
            <div className="text-left hidden md:block min-w-0">
              <h4 className="text-[14px] font-bold text-[var(--color-on-dark)] leading-tight truncate">{userDetails.firstName} {userDetails.lastName}</h4>
              <span className="text-[12px] text-[var(--color-muted)] flex items-center space-x-1 mt-0.5">
                ...
                <span className="truncate">{userDetails.designation}</span>
              </span>
            </div>
            <ChevronDown className="h-3 w-3 sm:h-4 sm:w-4 text-[var(--color-muted)] hidden sm:block shrink-0" />
          </button>
